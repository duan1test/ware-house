    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('common.report.transfer') }}</li>
            </ol>
        </nav>
    </div>
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between">
                <h5 class="page-title mt-3 mb-3">{{ __('common.report.transfer') }}</h5>
            ...
            <div class="" style="display: none;" id="filterFormContain" name="filterFormContain">
                <form action="" method="GET" {{ request()->all() ? 'block' : 'none' }} id="filterForm" name="filterForm">
                    <div class="row mb-3">
                        ...
                        <div class="col-3">
                            <label for="" class="font-medium font-14" >{{ __('attributes.transfer.ref') }}</label>
                            <input type="text" name="reference" id="reference" class="form form-control font-14 mt-1 mb-2" value="{{ old('reference', request('reference')) }}" placeholder="{{ __('attributes.transfer.ref') }}">
                        </div>
                        <div class="col-3 mt-1">
                            <label for="" class="font-medium font-14" >{{ __('attributes.transfer.warehouse_from') }}</label>
                            <select name="from_warehouse_id" id="warehouse_from" class="choices w-full mt-1 mb-2">
                                <option value="" disabled selected hidden>{{ __('common.transfer.select_warehouse') }}</option>
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}" {{ old('from_warehouse_id', request('from_warehouse_id')) == $warehouse->id ? 'selected' : '' }} >{{ $warehouse->name }} ({{ $warehouse->code }})</option> 
                                    @endforeach
                            </select>
                        </div>
                        <div class="col-3 mt-1">
                            <label for="" class="font-medium font-14" >{{ __('attributes.transfer.warehouse_to') }}</label>
                            <select name="to_warehouse_id" id="warehouse_to" class="choices w-full mt-1 mb-2">
                                <option value="" disabled selected hidden>{{ __('common.transfer.select_warehouse') }}</option>
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}" {{ old('to_warehouse_id', request('to_warehouse_id')) == $warehouse->id ? 'selected' : '' }} >{{ $warehouse->name }} ({{ $warehouse->code }})</option> 
                                    @endforeach
                            </select>
                        </div>
                        <div class="col-3 mt-1">
                    ...
                    <div class="d-flex justify-content-between">
                        <div>
                            <button type="reset" id="resetFilter" name="resetFilter" class="btn bg-gray-200 resetFilter">{{ __('attributes.transfer.reset') }}</button>
                        </div>
                        <div class="d-flex">
                            ...
                            <button type="" name="searchFilter" class="btn bg-gray-800 hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring focus:ring-gray-300 focus:border-gray-900 focus:shadow-outline-gray text-white btn-sm d-flex align-items-center justify-content-center searchFilter">{{ __('attributes.transfer.submit') }}</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="card-body">
            <div class="mt-1">
                <div>
                    @include('pages.report.table.table_list_transfer', ['transfers' => $transfers])
                </div>
            </div>
        </div>
    </div>
    ...
    @push('js')
        <script type="module">
            ...
            $('#resetFilter').on('click', function() {
                const baseUrl = "{{ route('reports.transfer') }}" + '?searchFilter';
