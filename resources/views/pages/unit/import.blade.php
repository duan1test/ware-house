    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('common.unit.title_import') }}</li>
            </ol>
        </nav>
    </div>
    <div class="card">
        <div class="card-header">
            <h5 class="page-title mt-3 mb-3">{{ __('common.unit.title_import') }}</h5>
            <div class="text-secondary page-description mb-3">{{ __('common.unit.text_import') }}</div>
        </div>

        <form style="display: contents;" method="POST" action="{{ route('units.import.save') }}" enctype="multipart/form-data">
            <div class="card-body">
                <div class="row">
                    <div>
                        @csrf
                            ...
                            <div class="input-container">
                                <div class="drop-zone text-center p-3">
                                    ...
                                    <p class="text-center text-secondary mt-3">{{ __('common.contact.import_columns', [
                                        'columns' => __('attributes.unit.name') . ', ' . __('attributes.unit.code') . ', ' . __('attributes.unit.base_unit'). ', ' . __('attributes.unit.operator'). ', ' . __('attributes.unit.operation_value')
                                    ]) }}</p>
                                    <p class="text-center text-secondary">{{ __('common.contact.import_data', [
                                        'columns' => __('attributes.unit.name') . ', ' . __('attributes.unit.code')
                                    ]) }}</p>
                                    @error('excel')
