    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('common.transfer.update') }}</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="card">
        <div class="card-header flex-column d-flex">
            <div class="d-flex align-items-center">
                <a href="{{ $filters['prv'] ?  url()->previous() : handleRoute(route: 'transfers.index', options: $filters) }}" class="me-2">
                    <i class='bx bx-arrow-back'></i>
                </a>
                <h5 class="page-title mt-3 mb-3">{{ __('common.transfer.update') }} ({{$transfer->reference}})</h5>
            </div>
                    ...
                    <div class="mt-3">
                        <div class="alert alert-warning d-flex flex-row justify-content-between align-items-center" role="alert">
                            <div class="flex justify-start items-center gap-2">
                                ...
                                <span>{{ __('attributes.transfer.soft_delete') }}</span>
                            </div>
                            <div class="d-flex justify-content-end">
                                <form method="POST" id="form-restore-{{ $transfer->id }}" action="{{ route('transfers.restore',  ['transfer' => $transfer->id]) }}" style="margin-right: 10px;">
                                    @csrf
                                    @method('PUT')
                                    <button type="button" data-id="{{ $transfer->id }}" class="btn btn-warning btn-sm form-restore">
                                        {{ __('common.restore') }}
                                    </button>
                                </form>
                            </div>
        ...
        <form method="POST" id="form-update" action="{{ route('transfers.update', $transfer->id) }}" style="display: contents;" enctype="multipart/form-data">
            <div class="card-body">
                <div class="row">           
                    @csrf
                    @method('put')
                    <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                        <div class="input-container">
                            ...
                            <input type="date" onchange="checkValue('error-date')" id="error-date" class="form-control" value="{{ old('date', $transfer->date) }}" placeholder="{{ __('attributes.transfer.date') }}" name="date">
                            <div class="error-date text-danger error-message hidden"></div>
                        </div>

                        <div class="input-container">
                            <label for="error-reference" class="form-label">
                                {{ __('attributes.transfer.ref') }}
                            </label>
                            <input type="text" onkeyup="checkValue('error-reference')" value="{{ old('reference', $transfer->reference) }}" id="error-reference" class="form-control placeholder="{{ __('attributes.transfer.ref') }}" name="reference">
                    ...
                    <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                        <div class="input-container">
                            <label for="error-from_warehouse_id" class="form-label">{{ __('attributes.transfer.warehouse_from') }}
                                <span class="text-danger">*</span>
                            </label>
                            <select name="from_warehouse_id" onchange="checkValue('error-from_warehouse_id')" id="error-from_warehouse_id" class="choices w-full">
                                ...
                                @else
                                    ...
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}" {{ $warehouse->id == old('from_warehouse_id', $transfer->from_warehouse_id) ? 'selected' : '' }}>{{ $warehouse->name }} ({{ $warehouse->code }})</option> 
                                    @endforeach
                                @endif
                            </select>
                            <div class="error-from_warehouse_id text-danger error-message hidden"></div>
                        </div>

                        <div class="input-container">
                            <label for="error-to_warehouse_id" class="form-label">{{ __('attributes.transfer.warehouse_to') }}
                                <span class="text-danger">*</span>
                            </label>
                            <select name="to_warehouse_id" onchange="checkValue('error-to_warehouse_id')" id="error-to_warehouse_id" class="choices w-full">
                                <option value="" disabled selected hidden>{{ __('common.transfer.select_warehouse') }}</option>
                                @foreach ($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}" {{ $warehouse->id == old('to_warehouse_id', $transfer->to_warehouse_id) ? 'selected' : '' }}>{{ $warehouse->name }} ({{ $warehouse->code }})</option> 
                                @endforeach
                            </select>
                            <div class="error-to_warehouse_id text-danger error-message hidden"></div>
                        </div>
                    </div>
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                    ...
                                    <table id="items-table" class="table">
                                        ...
                                        @foreach ($transfer->items as $item)
                                            ...
                                            @if (is_null($item->variations) || $item->variations->isEmpty())
                                                ...
                                                @endphp
                                                @include('pages.transfer.elements.item_alone', ['unitHtml' => $unitHtml, 'quantity' => $item->quantity, 'hasSer' => $item->item->has_serials, 'hasVar' => $item->item->has_variants, 'name' => ($item->item->name . ' (' . $item->item->code . ')'), 'id' => $item->item->id, 'trackWeight' => $trackWeight, 'variantString' => ($item->item->name . ' (' . $item->item->code . ')')])
                                            @else
                                                @include('pages.transfer.elements.item_together_render', ['quantity' => $item->quantity, 'hasSer' => $item->item->has_serials, 'hasVar' => $item->item->has_variants, 'itemId' => $item->item->id, 'htmlVariant' => '', 'name' => ($item->item->name . ' (' . $item->item->code . ')')])
                                                @foreach ($item->variations as $variant)
                                                    ...
                                                    @endphp
                                                    @include('pages.transfer.elements.item_variant_render', ['unitHtml' => $unitHtml, 'quantity' => $variant->pivot->quantity, 'variantId' => $variant->id, 'id' => $item->item->id, 'htmlVariants' => $variantName, 'trackWeight' => $trackWeight, 'variantString' => $sel])
                                                @endforeach
                                            @endif
                                        @endforeach
                        ...
                        <div class="input-container">
                            ...
                            <textarea name="details" id="details" class="form-control">{{ old('details', $transfer->details) }}</textarea>
                        </div>

                        <div class="form-check form-switch input-container d-flex p-0 align-items-center">
                            <label class="switch">
                                <input type="checkbox" name="draft" {{ old('draft', $transfer->draft) == 1 ? 'checked' : '' }} value="1"  id="draft">
        ...
        <div class="card-footer">
            <div class="text-end d-flex justify-content-between">
                <form id="form-delete" action="{{ !$transfer->deleted_at ? route('transfers.destroy', $transfer->id) : route('transfers.destroy.permanently', $transfer->id) }}" method="post">
                    @csrf
                    @method('delete')
    ...
    @push('js')
        <script type="module">
            $(document).ready(function () {
                ...
                const itemsJson = @json($transfer->items);
                ...
                $('.btn-update-item').on('click', function() {
                    $.ajax({
                        type: "PUT",
                        url: "{{ route('transfers.update', $transfer->id) }}",
                ...
                $(document).on('click', '.list-group-item', function () {
                    ...
                    ? `<input type="number" name="items[${itemId}][selected][variations][:variantId][weight]" class="form-control" required>` 
                    : (hasVariant == '0' && isWeight == '1' ? `<input type="number" name="items[${itemId}][weight]" class="form-control" required>` : '');
                    ...
                    if($(this).data('hasvariant') && isVariants) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                $.each(variant, function(key, value) {
                                    ...
                                    childVariant += `@include('pages.transfer.elements.child_variant', ['variantOption' => '${options}', 'name' => '${value.name}'])`;
                                });
                                const html = `@include('pages.transfer.elements.alert_variant', ['childVariant' => '${childVariant}'])`;
                                Swal.fire({
                                    ...
                                    customClass: {
                                        title: 'd-flex',
                                        actions: 'w-100 justify-content-end'
                                    },
                                    preConfirm: async () => {
                                        let row = '';
                                        let sel = '';
                                        ...
                                        if (!objChild[itemId].includes(sel) && !$(`#item-${itemId}`).length) {
                                            let a= ''; let b = '';
                                            const variants = `@include('pages.transfer.elements.item_variant', ['unitHtml' => '${unitHtml}','variantId' => '${variantId}', 'id' => '${itemId}', 'htmlVariants' => '${htmlVariants}', 'trackWeight' => '${trackWeight}', 'variantString' => '${sel}'])`;
                                            row += `@include('pages.transfer.elements.item_together', ['hasSer' => '${hasSerial}', 'hasVar' => '${hasVariant}', 'itemId' => '${itemId}', 'htmlVariant' => '${variants}', 'name' => '${itemName}'])`;
                                            objChild[itemId].push(sel);
                                        }
                                        if(!objChild[itemId].includes(sel) && $(`#item-${itemId}`).length) {
                                            row += `@include('pages.transfer.elements.item_variant', ['unitHtml' => '${unitHtml}','variantId' => '${variantId}', 'id' => '${itemId}', 'htmlVariants' => '${htmlVariants}', 'trackWeight' => '${trackWeight}', 'variantString' => '${sel}'])`;
                                            objChild[itemId].push(sel);
                                        }
                                        return row;
                                    },
                        ...
                        if(!objParent[itemId].includes(`${itemName}`)) {         
                            row += `@include('pages.transfer.elements.item_alone', ['unitHtml' => '${unitHtml}','hasSer' => '${hasSerial}', 'hasVar' => '${hasVariant}', 'name' => '${itemName}', 'id' => '${itemId}', 'trackWeight' => '${trackWeight}', 'variantString' => '${itemName}'])`;
