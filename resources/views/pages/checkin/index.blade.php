    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('common.checkin.index') }}</li>
            </ol>
        </nav>
    </div>
    <div class="card">
        <div class="card-header">
            <h5 class="page-title mt-3 mb-3">{{ __('common.checkin.index') }}</h5>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                    <div class="table-filter mt-2 d-flex justify-content-between align-items-center" id="checkin-filters">
                        ...
                        <div class="btn-group">
                            <a class="btn-item me-0" title="{{ __('common.checkin.create') }}" href="{{ route('checkins.create') }}">
            ...
            <div class="mt-2">
                <x-handle-ajax url="{{ route('checkins.index') }}">
                    @include('pages.checkin.table_list_checkin', ['checkins' => $checkins])
                </x-handle-ajax>
            </div>
        </div>
    </div>
    @push('js')
        <script type="module">
            $(document).ready(function () {
                const handleAjax = () => {
                    const $searchEL = $('#checkin-filters input[name=search]');
                    const $statusEL = $('#checkin-filters select[name=trashed]');
                    const $draftEL = $('#checkin-filters select[name=draft]');
                    const $resetEL = $('#checkin-filters .btn-reset-filter');
                ...
                $(document).on('click', '.soft-delete', function(e) {
                    ...
                    let deleteFormHtml = `
                       <form id="swal-delete-form" method="POST" action="${isPermanentDelete ? '{{ route('checkins.destroy.permanently', ':id') }}'.replace(':id', id) : '{{ route('checkins.destroy', ':id') }}'.replace(':id', id)}">
