    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('common.adjustment.update') }}</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="card">
        <div class="card-header">
           <div class="flex items-center justify-start">
                <a href="{{ $filters['prv'] ?  url()->previous() : route('adjustments.index', $filters) }}" class="me-2">
                    <i class='bx bx-arrow-back'></i>
                </a>
                <h5 class="page-title mt-3 mb-3">{{ __('common.adjustment.update') }} ({{$adjustment->reference}})</h5>
           </div>
                ...
                <div class="mt-3">
                    <div class="alert alert-warning d-flex flex-row justify-content-between align-items-center" role="alert">
                        <div class="flex justify-start items-center gap-2">
                            ...
                            <span>{{ __('attributes.adjustment.soft_delete') }}</span>
                        </div>
                        <div class="d-flex justify-content-end">
                            <form method="POST" id="form-restore-{{ $adjustment->id }}" action="{{ route('adjustments.restore',  ['adjustment' => $adjustment->id]) }}" style="margin-right: 10px;">
                                @csrf
                                @method('PUT')
                                <button type="button" data-id="{{ $adjustment->id }}" class="btn btn-warning btn-sm form-restore">
                                    {{ __('common.restore') }}
                                </button>
                            </form>
                        </div>
        ...
        <form method="POST" id="form-update" action="{{ route('adjustments.update',$adjustment->id) }}" style="display: contents;" enctype="multipart/form-data">
            <div class="card-body">
                <div class="row">
                    @csrf
                    @method('PUT')
                    <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                        <div class="input-container">
                            ...
                            <input type="date" onchange="checkValue('error-date')" id="error-date" class="form-control" value="{{ old('date', \Carbon\Carbon::now()->toDateString()) }}" placeholder="{{ __('attributes.transfer.date') }}" name="date">
                        ...
                        <div class="input-container">
                            ...
                            <input type="text" onkeyup="checkValue('error-date')" value="{{ old('reference',$adjustment->reference) }}" id="error-reference" class="form-control" placeholder="{{ __('attributes.adjustment.ref') }}" name="reference">
                    ...
                    <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                        <div class="input-container">
                            <label for="type" class="form-label">{{ __('attributes.adjustment.type') }}
                                <span class="text-danger">*</span>
                            </label>
                            <select name="type" id="type" onchange="checkValue('error-type')" class="choices w-full">
                                <option value="" disabled selected hidden>{{ __('attributes.adjustment.type') }}</option>
                                <option {{$adjustment->type=='Damage'?'selected':''}} value="Damage" >{{ __('common.adjustment.damage') }}</option>
                                <option {{$adjustment->type=='Addition'?'selected':''}} value="Addition" >{{ __('common.adjustment.addition') }}</option>
                                <option {{$adjustment->type=='Subtraction'?'selected':''}} value="Subtraction" >{{ __('common.adjustment.subtraction') }}</option>
                            </select>
                            <div class="error-type text-danger error-message hidden"></div>
                        </div>

                        <div class="input-container">
                            <label for="warehouse" class="form-label">{{ __('attributes.adjustment.warehouse') }}
                                <span class="text-danger">*</span>
                            </label>
                            <select name="warehouse_id" onchange="checkValue('error-warehouse_id')" id="warehouse" class="choices w-full">
                                ...
                                @else
                                    ...
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}" {{ $warehouse->id == old('warehouse_id', $adjustment->warehouse_id) ? 'selected' : '' }}>{{ $warehouse->name }} ({{ $warehouse->code }})</option> 
                                    @endforeach
                                @endif
                            </select>
                    ...
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                    ...
                                    <table id="items-table" class="table">
                                        ...
                                        @foreach ($adjustment->items as $item)
                                            ...
                                            @if (is_null($item->variations) || $item->variations->isEmpty())
                                                ...
                                                @endphp
                                                @include('pages.adjustment.elements.item_alone', ['unitHtml' => $unitHtml, 'quantity' => $item->quantity, 'hasSer' => $item->item->has_serials, 'hasVar' => $item->item->has_variants, 'name' => ($item->item->name . ' (' . $item->item->code . ')'), 'id' => $item->item->id, 'trackWeight' => $trackWeight, 'variantString' => ($item->item->name . ' (' . $item->item->code . ')')])
                                            @else
                                                @include('pages.adjustment.elements.item_together_render', ['quantity' => $item->quantity, 'hasSer' => $item->item->has_serials, 'hasVar' => $item->item->has_variants, 'itemId' => $item->item->id, 'htmlVariant' => '', 'name' => ($item->item->name . ' (' . $item->item->code . ')')])
                                                @foreach ($item->variations as $variant)
                                                    ...
                                                    @endphp
                                                    @include('pages.adjustment.elements.item_variant_render', ['unitHtml' => $unitHtml, 'quantity' => $variant->pivot->quantity, 'variantId' => $variant->id, 'id' => $item->item->id, 'htmlVariants' => $variantName, 'trackWeight' => $trackWeight, 'variantString' => $sel])
                                                @endforeach
                                            @endif
                                        @endforeach
                                        <tbody class="instruct hidden">
                                            <tr>
                                                <td colspan="5" class="text-black">{{ __('common.items.instruct') }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                        ...
                        <div class="input-container">
                            ...
                            <textarea name="details" id="details" class="form-control">{{ old('details',$adjustment->details) }}</textarea>
                        </div>
        ...
        <div class="card-footer">
            <div class="text-end d-flex justify-content-between">
                <form id="form-delete" action="{{ !$adjustment->deleted_at ? route('adjustments.destroy', $adjustment->id) : route('adjustments.destroy.permanently', $adjustment->id) }}" method="post">
                    @csrf
                    @method('delete')
        ...
        <script type="module">
            $(document).ready(function () {
                ...
                $('.btn-update-item').on('click', function() {
                    $.ajax({
                        type: "PUT",
                        url: "{{ route('adjustments.update',$adjustment->id) }}",
                ...
                const itemsJson = @json($adjustment->items);
