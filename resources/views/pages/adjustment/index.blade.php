    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('common.adjustment.index') }}</li>
            </ol>
        </nav>
    </div>
    <div class="card">
        <div class="card-header">
            <h5 class="page-title mt-3 mb-3">{{ __('common.adjustment.index') }}</h5>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                    <div class="table-filter mt-2 d-flex justify-content-between align-items-center" id="adjustment-filters">
                        <div class="table-search d-flex">
                            <div class="input-group input-group-sm">
                                <button class="btn btn-light dropdown-toggle" title="{{__('common.filter')}}" type="button" data-bs-toggle="dropdown" aria-expanded="false"><i
                        ...
                        <div class="btn-group">
                            <a class="btn-item me-0" title="{{ __('common.adjustment.create') }}" href="{{ route('adjustments.create') }}">
            ...
            <div class="mt-2">
                <x-handle-ajax url="{{ route('adjustments.index') }}">
                    @include('pages.adjustment.table_list_adjustment', ['adjustments' => $adjustments])
                </x-handle-ajax>
            </div>
        </div>
    </div>
    @push('js')
        <script type="module">
            $(document).ready(function () {
                const handleAjax = () => {
                    const $searchEL = $('#adjustment-filters input[name=search]');
                    const $statusEL = $('#adjustment-filters select[name=trashed]');
                    const $draftEL = $('#adjustment-filters select[name=draft]');
                    const $resetEL = $('#adjustment-filters .btn-reset-filter');
                ...
                $(document).on('click', '.soft-delete', function(e) {
                    ...
                    let deleteFormHtml = `
                       <form id="swal-delete-form" method="POST" action="${isPermanentDelete ? '{{ route('adjustments.destroy.permanently', ':id') }}'.replace(':id', id) : '{{ route('adjustments.destroy', ':id') }}'.replace(':id', id)}">
                ...
                $(document).on('click', '.clickable-row', async function() {
                    ...
                    let url = "{{ route('adjustments.show', ['adjustment' => ':id', 'json' => 1]) }}";
                ...
                function renderDetail(data) {
                        ...
                        <div class="card-body overflow-hidden">
                            ...
                            <div class="row d-flex flex-row justify-content-center align-items-center mt-4">
                                <div class="col-12 d-flex flex-column text-start">
                                    <span class="fz-16 lh-lg">{{ __('attributes.adjustment.type') }}: ${formatType(data.type)}</span>
                                    <span class="fz-16 lh-lg">{{ __('attributes.transfer.date') }}: ${formatDate(data.date)}</span>
                                    <span class="fz-16 lh-lg">{{ __('attributes.adjustment.ref') }}: ${data.reference}</span>
                    ...
                    `;
                    Swal.fire({
                        title: `{{ __('common.adjustment.detail') }} (${data.reference})`,
                        html: `
                            <div class="row">
                                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                ${html}
