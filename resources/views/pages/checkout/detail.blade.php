<x-app-layout>
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('common.checkout.detail') }}</li>
                </ol>
            </nav>
        </div>
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <div class="w-100 d-flex align-items-center">
                    <a href="{{ route('checkouts.index', $filters) }}" class="me-2">
                        <i class='bx bx-arrow-back'></i>
                    </a>
                    <h5 class="page-title mt-3 mb-3">{{ __('common.checkout.detail') }} ({{$checkout->reference}})</h5>
                </div>
                <div class="me-2">
                ...
                <div>
                    <a href="{{ route('checkouts.edit', [$checkout->id, ...$filters]) }}" class="item-detail-edit rounded-circle p-2" title="{{ __('common.checkout.edit') }}">
            ...
            <div class="card-body overflow-hidden printable">
                ...
                <div class="d-flex flex-column text-end">
                    <span class="font-bold lh-lg">{{ $checkout->warehouse->name }} ({{ $checkout->warehouse->code }})</span>
                    <span class="lh-lg">{{ $checkout->warehouse->address ?? '' }}</span>
                    <span class="lh-lg">{{ $checkout->warehouse->phone ?? '' }}</span>
                    <span class="lh-lg">{{ $checkout->warehouse->email ?? '' }}</span>
                </div>
                ...
                <div class="row d-flex flex-row justify-content-center align-items-center mt-4">
                    <div class="col-6 d-flex flex-column text-start">
                        <span class="lh-lg">{{ __('attributes.transfer.date') }}: {{ \Carbon\Carbon::parse($checkout->date)->format('d/m/Y') }}</span>
                        <span class="lh-lg">{{ __('attributes.checkout.ref') }}: {{ $checkout->reference }}</span>
                        <span class="lh-lg">{{ __('attributes.transfer.create_at') }}: {{ \Carbon\Carbon::parse($checkout->created_at)->format('d/m/Y, H:i:s') }}</span>
                    </div>
                    <div class="col-6 d-flex flex-column text-start">
                        <span class="font-bold lh-lg">{{__('attributes.checkin.contact')}}:</span>
                    <span class="lh-lg">{{ $checkout->contact->name }}</span>
                    <span class="lh-lg">{{ $checkout->contact->address ?? '' }}</span>
                    <span class="lh-lg">{{ $checkout->contact->phone ?? '' }}</span>
                    <span class="lh-lg">{{ $checkout->contact->email ?? '' }}</span>
                    </div>
                </div>
    ...
    @push('js')
        <script type="module">
            JsBarcode("#barcode", "{{ $checkout->reference }}", {
                format: 'CODE128',
                lineColor: "#000000",
                width: 2,
                height: 100,
