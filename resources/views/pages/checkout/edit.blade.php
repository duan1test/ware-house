    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item"><a href="{{ route('checkouts.index') }}">{{ __('common.checkout.index') }}</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ __('common.checkout.edit') }}</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="card">
        <div class="card-header flex-column d-flex">
            <div class="d-flex align-items-center">
                <a href="{{ $filters['prv'] ?  url()->previous() : handleRoute(route: 'checkouts.index', options: $filters) }}" class="me-2">
                    <i class='bx bx-arrow-back'></i>
                </a>
                <h5 class="page-title mt-3 mb-3">{{ __('common.checkout.edit') }} ({{$checkout->reference}})</h5>
            </div>
            @if ($checkout->deleted_at != null)
                <div class="row">
                    <div class="mt-3">
                        <div class="alert alert-warning d-flex flex-row justify-content-between align-items-center" role="alert">
                            <div class="flex justify-start items-center gap-2">
                                ...
                                <span>{{ __('attributes.checkout.soft_delete') }}</span>
                            </div>
                            <div class="d-flex justify-content-end">
                                <form method="POST" id="form-restore-{{ $checkout->id }}" action="{{ route('checkouts.restore',  ['checkout' => $checkout->id]) }}" style="margin-right: 10px;">
                                    @csrf
                                    @method('PUT')
                                    <button type="button" data-id="{{ $checkout->id }}" class="btn btn-warning btn-sm form-restore">
                                        {{ __('common.restore') }}
                                    </button>
                                </form>
                            </div>
        ...
        <form method="POST" id="form-update" action="{{ route('checkouts.update',$checkout->id) }}" style="display: contents;" enctype="multipart/form-data">
            <div class="card-body">
                <div class="row">       
                    @csrf
                    @method('PUT')
                    <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                        ...
                        <div class="input-container">
                            <label for="error-reference" class="form-label">
                                {{ __('attributes.checkout.ref') }}
                            </label>
                            <input type="text" onkeyup="checkValue('error-reference')" value="{{ old('reference',$checkout->reference) }}" id="error-reference" class="form-control" placeholder="{{ __('attributes.checkout.ref') }}" name="reference">
                    ...
                    <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                        <div class="input-container">
                            ...
                            <select name="contact_id" onchange="checkValue('error-contact_id')" id="error-contact_id" class="choices">
                                <option value="" disabled selected hidden>{{ __('attributes.checkout.contact') }}</option>
                                @foreach ($contacts as $contact)
                                    <option {{ old('contact_id',$checkout->contact_id) == $contact->id ? 'selected' : '' }} value="{{ $contact->id }}" >{{ $contact->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="input-container">
                            ...
                            <select name="warehouse_id" onchange="checkValue('error-warehouse_id')" id="error-warehouse_id" class="choices">
                                ...
                                @else
                                    ...
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}" {{ $warehouse->id == old('warehouse_id', $checkout->warehouse_id) ? 'selected' : '' }}>{{ $warehouse->name }} ({{ $warehouse->code }})</option> 
                                    @endforeach
                                @endif
                            </select>
                    ...
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                    ...
                                    <table id="items-table" class="table">
                                        ...
                                        @foreach ($checkout->items as $item)
                                            ...
                                            @if (is_null($item->variations) || $item->variations->isEmpty())
                                                ...
                                                @endphp
                                                @include('pages.checkout.elements.item_alone', ['unitHtml' => $unitHtml, 'quantity' => $item->quantity, 'hasSer' => $item->item->has_serials, 'hasVar' => $item->item->has_variants, 'name' => ($item->item->name . ' (' . $item->item->code . ')'), 'id' => $item->item->id, 'trackWeight' => $trackWeight, 'variantString' => ($item->item->name . ' (' . $item->item->code . ')')])
                                            @else
                                                @include('pages.checkout.elements.item_together_render', ['quantity' => $item->quantity, 'hasSer' => $item->item->has_serials, 'hasVar' => $item->item->has_variants, 'itemId' => $item->item->id, 'htmlVariant' => '', 'name' => ($item->item->name . ' (' . $item->item->code . ')')])
                                                @foreach ($item->variations as $variant)
                                                    ...
                                                    @endphp
                                                    @include('pages.checkout.elements.item_variant_render', ['unitHtml' => $unitHtml, 'quantity' => $variant->pivot->quantity, 'variantId' => $variant->id, 'id' => $item->item->id, 'htmlVariants' => $variantName, 'trackWeight' => $trackWeight, 'variantString' => $sel])
                                                @endforeach
                                            @endif
                                        @endforeach
                                        <tbody class="instruct hidden">
                                            <tr>
                                                <td colspan="5" class="text-black">{{ __('common.checkout.instruct') }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                        ...
                        <div class="input-container">
                            ...
                            <textarea name="details" id="details" class="form-control">{{ old('details',$checkout->details) }}</textarea>
                        </div>

                        <div class="form-check form-switch input-container d-flex p-0 align-items-center">
                            <label class="switch">
                                <input type="checkbox" name="draft" {{ old('draft',$checkout->draft) == 1 ? 'checked' : '' }} value="1"  id="draft">
        ...
        <div class="card-footer">
            <div class="text-end d-flex justify-content-between">
                <form id="form-delete" action="{{ !$checkout->deleted_at ? route('checkouts.destroy', $checkout->id) : route('checkouts.destroy.permanently', $checkout->id) }}" method="post">
                    @csrf
                    @method('delete')
    ...
    @push('js')
        <script type="module">
            $(document).ready(function () {
                ...
                const attachmentArray = @json($checkout->attachments);
                ...
                const itemsJson = @json($checkout->items);
                ...
                $('.btn-update-item').on('click', function() {
                    $.ajax({
                        type: "PUT",
                        url: "{{ route('checkouts.update',$checkout->id) }}",
                ...
                $(document).on('click', '.list-group-item', function () {
                    ...
                    if($(this).data('hasvariant') && isVariants) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                $.each(variant, function(key, value) {
                                    ...
                                    childVariant += `@include('pages.checkout.elements.child_variant', ['variantOption' => '${options}', 'name' => '${value.name}'])`;
                                });
                                const html = `@include('pages.checkout.elements.alert_variant', ['childVariant' => '${childVariant}'])`;
                                Swal.fire({
                                    ...
                                    preConfirm: async () => {
                                        ...
                                        if (!objChild[itemId].includes(sel) && !$(`#item-${itemId}`).length) {
                                            let a= ''; let b = '';
                                            const variants = `@include('pages.checkout.elements.item_variant', ['unitHtml' => '${unitHtml}','variantId' => '${variantId}', 'id' => '${itemId}', 'htmlVariants' => '${htmlVariants}', 'trackWeight' => '${trackWeight}', 'variantString' => '${sel}'])`;
                                            row += `@include('pages.checkout.elements.item_together', ['trackWeight' => '${isTrachWeight}', 'hasSer' => '${hasSerial}', 'hasVar' => '${hasVariant}', 'itemId' => '${itemId}', 'htmlVariant' => '${variants}', 'name' => '${itemName}'])`;
                                            objChild[itemId].push(sel);
                                        }
                                        if(!objChild[itemId].includes(sel) && $(`#item-${itemId}`).length) {
                                            row += `@include('pages.checkout.elements.item_variant', ['unitHtml' => '${unitHtml}','variantId' => '${variantId}', 'id' => '${itemId}', 'htmlVariants' => '${htmlVariants}', 'trackWeight' => '${trackWeight}', 'variantString' => '${sel}'])`;
                                            objChild[itemId].push(sel);
                                        }
                                        return row;
                                    },
                        ...
                        if(!objParent[itemId].includes(`${itemName}`)) {
                            row += `@include('pages.checkout.elements.item_alone', ['unitHtml' => '${unitHtml}','hasSer' => '${hasSerial}', 'hasVar' => '${hasVariant}', 'name' => '${itemName}', 'id' => '${itemId}', 'trackWeight' => '${trackWeight}', 'variantString' => '${itemName}'])`;
