    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('common.checkout.index') }}</li>
            </ol>
        </nav>
    </div>
    <div class="card">
        <div class="card-header">
            <h5 class="page-title mt-3 mb-3">{{ __('common.checkout.index') }}</h5>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                    <div class="table-filter mt-2 d-flex justify-content-between align-items-center" id="checkout-filters">
                        ...
                        <div class="btn-group">
                            <a class="btn-item me-0" title="{{ __('common.checkout.create') }}" href="{{ route('checkouts.create') }}">
            ...
            <div class="mt-2">
                <x-handle-ajax url="{{ route('checkouts.index') }}">
                    @include('pages.checkout.table_list_checkout', ['checkouts' => $checkouts])
                </x-handle-ajax>
            </div>
        </div>
    </div>
    @push('js')
        <script type="module">
            $(document).ready(function () {
                const handleAjax = () => {
                    const $searchEL = $('#checkout-filters input[name=search]');
                    const $statusEL = $('#checkout-filters select[name=trashed]');
                    const $draftEL = $('#checkout-filters select[name=draft]');
                    const $resetEL = $('#checkout-filters .btn-reset-filter');
                    ...
                    function closeFilter() {
                        $('#adjustment-filters .btn-show-filter').click();
                    }
                }
                handleAjax();
                ...
                $(document).on('click', '.clickable-row', async function() {
                    ...
                    let url = "{{ route('checkouts.show', ['checkout' => ':id', 'json' => 1]) }}";
                ...
                function renderDetail(data) {
                    ...
                    $.each(data.items, function (key, val) {
                        if(val.variations == null || val.variations.length == 0) {
                            ...
                            `<tr class="group avoid border-b">
                                ...
                                <td class="group-hover:bg-gray-100 border-t px-6 py-2 text-center">${formatDecimalNumber(val.quantity,data.setting.fraction,data.setting.default_locale) || '0.00'} ${val?.unit?.code || ''}</td>
                            </tr>`;
                        }
                        else {
                            htmlItem += 
                        ...
                        htmlAlert += `<div class="row">
                                        <div class="col-12">
                                            <div class="alert alert-warning text-start fz-16">
                                                ...
                                                {{ __('attributes.checkout.soft_delete') }}
                                            </div>
                                        </div>
                                    </div>`;
                    }
                    ...
                    <div class="card">
                        ...
                        <div class="card-body overflow-hidden">
                            ...
                            <div class="d-flex flex-col justify-content-center align-items-center">
                                <p class="text-xl text-center uppercase font-bold">{{__('common.checkout.title')}}</p>
                            ...
                            <div class="row d-flex flex-row justify-content-center align-items-center mt-4">
                                <div class="col-6 d-flex flex-column text-start">
                                    ...
                                    <span class="fz-16 lh-lg">{{ __('attributes.checkout.ref') }}: ${data.reference}</span>
                            ...
                            <div class="mb-4 text-start mt-2 attachments">
                                <label for="formFile" class="fz-16">
                                {{ __('attributes.transfer.attachment') }}
                                </label>
                               <input type="file" hidden name="filepond">
                            </div>
                    ...
                    `;
                    Swal.fire({
                        title: `{{ __('common.checkout.detail') }} (${data.reference})`,
                        html: `
                            <div class="row">
                                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                ${html}
                ...
                $(document).on('click', '.soft-delete', function(e) {
                    ...
                    let deleteFormHtml = `
                       <form id="swal-delete-form" method="POST" action="${isPermanentDelete ? '{{ route('checkouts.destroy.permanently', ':id') }}'.replace(':id', id) : '{{ route('checkouts.destroy', ':id') }}'.replace(':id', id)}">
