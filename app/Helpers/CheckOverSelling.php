        ...
        foreach ($order_items as $key => $order_item) {
            ...
            if (isset($order_item['selected']) && isset($order_item['selected']['variations']) && !empty($order_item['selected']['variations'])) {
                foreach ($order_item['selected']['variations'] as $order_variation) {
                    ...
                    if (!$variationStock) {
                        $error["items.{$key}.quantity"] = __choice(__('messages.transfer.variant'), ['name' => $order_item['name'], 'quantity' => $quantity . ' ' . ($unit ? $unit->name : $item->unit->name), 'available' => '0 ' . $item->unit->name, 'variant' => implode(', ', $meta)]);
                    } elseif ($base_quantity && $variationStock->quantity < $base_quantity) {
                        $error["items.{$key}.quantity"] = __choice(__('messages.transfer.variant'), ['name' => $order_item['name'], 'quantity' => $quantity . ' ' . ($unit ? $unit->name : $item->unit->name), 'available' => ((float) $variationStock->quantity) . ' ' . $item->unit->name, 'variant' => implode(', ', $meta)]);
                    }
                }
            } else {
                ...
                $base_quantity = $unit ? convert_to_base_quantity($quantity, $unit) : $quantity;
                if (!$stock && !empty($error)) {
                    $error["items.{$key}.quantity"] = __choice(__('messages.transfer.quantity'), ['name' => $order_item['name'], 'quantity' => $quantity . ' ' . ($unit ? $unit->name : $item->unit->name), 'available' => '0 ' . $item->unit->name]);
                } elseif ($base_quantity && $stock->quantity < $base_quantity) {
                    $error["items.{$key}.quantity"] = __choice(__('messages.transfer.quantity'), ['name' => $order_item['name'], 'quantity' => $quantity . ' ' . ($unit ? $unit->name : $item->unit->name), 'available' => ((float) $stock->quantity) . ' ' . $item->unit->name]);
                }
            }
        }
        return $error;
